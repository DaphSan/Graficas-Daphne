...
//identificador del manager de shader (ShaderProgram).
...
void Initialize()
{
	...
	//Leemos el codigo fuente del vertex shader con la clase auxiliar de InputFile
	...
	//Hacemos un cast (conversion de tipo de dato) porque no le podemos mandar strings a OpenGL
	//necesitamos mandarle const GLchar*
	const GLchar *vertexSource_c =
		(const GLchar*)vertexSource.c_str(); //convierte un tipo de dato a otro
	//Creamos un shader de tipo  Vertex Shader y guardamos su id
	...
	//Le pedimo que lo compile
	glCompileShader(vertexShaderHandle);


	//AQUI VA EL CORRECTOR
	...
	glGetShaderiv(fragmentShaderHandle, GL_COMPILE_STATUS, &fragmentShaderCompileSuccess);
	if (fragmentShaderCompileSuccess == GL_FALSE)
	{
		...
		{
			std::vector<GLchar> compileLog(logLength);
			glGetShaderInfoLog(fragmentShaderHandle, logLength, &logLength, &compileLog[0]);
		...
		std::cout << "Shader Default.frag did not compiled" << std::endl;
	}




	//creo el manager de los shaders. Almacenamos el id en la variable shaderProgram
	shaderProgram = glCreateProgram();
	//Hay que hacer un attach por cada shader, administra el vertex shader con identificador vertexShaderHandle
	glAttachShader(shaderProgram, vertexShaderHandle);
	glAttachShader(shaderProgram, fragmentShaderHandle);

	glBindAttribLocation(shaderProgram, 0, "VertexPosition");
	//En este shader el buffer con indice 1 lo asigno a la variable que se llama vertex color
	//asociamos el indice del buffer (VBO) con un nombre de variable en el shader.
...
void MainLoop()
{
	...
	// Paso 1 no lo podemos hacer aún. AHORA SIIII ~O~
	//ACTIVA EL PROGRAMA SHADER(MANAGER)
	...
	glBindVertexArray(0);
	// Paso 5 aún no lo podemos hacer. AHORA SIII
	glUseProgram(0);
